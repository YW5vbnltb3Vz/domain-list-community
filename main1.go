...
func (p *Processor) exportGFWList(exportedName string) error {
	pl, exist := p.parsedListByName[strings.ToUpper(exportedName)]
	if !exist || len(pl.FinalEntries) == 0 {
		return fmt.Errorf("'%s' list does not exist or is empty.", exportedName)
	}
	targetList := pl.FinalEntries
	var entryBytes []byte
...
func run() error {
	...
	// Export GfwList
	if _, exist := processor.parsedListByName["GEOLOCATION-!CN"]; exist {
		if err := processor.exportGFWList("GEOLOCATION-!CN"); err != nil {
